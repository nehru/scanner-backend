...
class AsyncJavaScriptEngine(BaseLanguageEngine):
    """Async JavaScript scanner engine"""
    ...
    def __init__(self, config_manager: AsyncLanguageConfigManager, semgrep_rules_base_dir: str = None, base_config: Optional[Dict] = None):
        super().__init__('javascript', None, base_config)
        self.config_manager = config_manager
        asyncio.create_task(self._configure_rule_loader(semgrep_rules_base_dir))
        
    async def _initialize_engine(self):
        self.language_config = await self.config_manager.get_language_config('javascript')
        if not self.language_config:
            raise ValueError("JavaScript language configuration not found")
    ...
    async def _configure_rule_loader(self, semgrep_rules_base_dir: str = None):
        ...
        language_configs = {
            'javascript': {
                'semgrep_rule_paths': ['javascript/security'],
